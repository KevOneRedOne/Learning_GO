...
func EntreeUtilisateur(color string) []int {
	...
	for index := 0; index < len(acolorier); index++ {
		...
		if len(acolorier) > len(flag.Args()[0]) {
			fmt.Print("\033[H\033[2J")
			fmt.Printf("MAUVAIS CHOIX\nVous devez coloriez : 	%s\nVous avez entré : 	%s\n\nRecommencer : ", flag.Args()[0], acolorier)
			scanner = bufio.NewScanner(os.Stdin)
			scanner.Scan()
			acolorier = scanner.Text()
			reset = true
		...
		if acolorier[index] == flag.Args()[0][index] {
			...
			if cmpt >= 4 {
				fmt.Print("\033[H\033[2J")
				fmt.Printf("MAUVAIS CHOIX\nVous devez coloriez : 	%s\nVous avez entré : 	%s\n\nPour rappel vous devez :\n - Taper les lettres que vous voulez colorier\n - Laissez des espaces pour les lettres que vous ne voulez pas colorier\n", flag.Args()[0], acolorier)
				fmt.Println("Exemple : ")
				fmt.Println("Couleur")
				fmt.Println("Cou eur")
...
func ReadFile(StylizedFile string) []string {
	...
	scanner.Split(bufio.ScanLines)    // sets-up scanner's preference to read the file line-by-line
...
//AsciiFS returns a filepath with the part of the program Ascii-FS
func AsciiFS() string {
	var filepath string        //value which stocks the path of the file
	if len(flag.Args()) == 2 { // lenght of arguments is os.Args[0,1,2] = 3 => "main.go" "word" "font-style"
	...
	} else if len(flag.Args()) == 1 { // lenght of arguments is os.Args[0,1] = 2 => "main.go" "word"
		return "standard.txt"
...
func main() {
	...
	_, ok := colors[color]
	if !ok { // Checks if the user chooses the right color
		fmt.Println("Error")
		os.Exit(0)
	}

	if len(flag.Args()) == 2 || len(flag.Args()) == 1 { // lenght of arguments is os.Args[0,1] = 2 => "main.go" "word"
		filepath := AsciiFS()
